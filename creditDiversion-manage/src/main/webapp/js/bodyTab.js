...
layui.define(["element", "jquery"], function (exports) {
    var element = layui.element,
        ...
        Tab = function () {
            this.tabConfig = {
                ...
                resource: undefined  //获取菜单json地址
            }
        };

    //获取二级菜单数据
    Tab.prototype.render = function () {
        var data = this.tabConfig.resource;
        //显示左侧菜单
        if ($(".navBar").html() == '') {
            var _this = this;
    ...
    Tab.prototype.tabAdd = function (_this) {
        ...
        if (_this.attr("target") == "_blank") {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                menu.push(curmenu);
                localStorage.setItem("menu", JSON.stringify(menu)); //打开的窗口
                ...
                var jsonCurmenu;
                if (localStorage.getItem("curmenu") != null && localStorage.getItem("curmenu") != '' && localStorage.getItem("curmenu") != "undefined") {
                    jsonCurmenu = JSON.parse(localStorage.getItem("curmenu"));
                }
                //当前窗口内容
                var curmenu = {
                    "icon": _this.find("i.iconfont").attr("data-icon") != undefined ? _this.find("i.iconfont").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
                ...
                localStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
                element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
                if (jsonCurmenu != null && jsonCurmenu.href != curmenu.href && curmenu.title != '后台首页') {//打开的不是当前显示的窗口
                    if ($(".clildFrame .layui-tab-item iframe[src='" + curmenu.href + "']")[0] != null) {
                        $(".clildFrame .layui-tab-item iframe[src='" + curmenu.href + "']")[0].contentWindow.location.reload(true);
                    }
                }
                if (curmenu.title == '后台首页') {
                    $("#main_div iframe")[0].contentWindow.location.reload(true);
                }
                that.tabMove(); //顶部窗口是否可滚动
            }
        }
    ...
    $("body").on("click", ".top_tab li", function () {
        ...
        if ($(this).index() == 0) {
            ...
            if (localStorage.getItem("curmenu") == "undefined") {
                ...
                if (curNav != JSON.stringify(delMenu)) {
                    ...
                    localStorage.setItem("curmenu", JSON.stringify(menu[liIndex - 1]));
                }
            }
        }
        ...
        //tab切换时刷新当前
        if (classStr != "layui-this") {
            var data_id = $(this).find(".layui-tab-close").eq(0).attr("data-id");
            if (data_id == null || data_id == '') {
                $("#main_div iframe")[0].contentWindow.location.reload(true);
            } else {
                if ($(".clildFrame .layui-tab-item iframe[data-id='" + data_id + "']")[0] != null) {
                    $(".clildFrame .layui-tab-item iframe[data-id='" + data_id + "']")[0].contentWindow.location.reload(true);
                }
            }
        }
        // new Tab().tabMove();
