...
layui.config({
    ...
    if(id!=0){
        //加载数据
        $.get("/manage/tmanageDepartmentInfo/selectOne", {id: id}, function (result) {
            if (result.code == 1) {
    ...
    form.verify({
        departmentName: function(value, item){ //value：表单的值、item：表单的DOM对象
            if(value===null||value===""){
                return '名称不能为空'
            }
    ...
    function submit(data,index){
        var url="/manage/tmanageDepartmentInfo/update";
        if(data.field.id==0){
            url="/manage/tmanageDepartmentInfo/add";
        }
        $.ajax({
            url:url,
            data: data.field,
