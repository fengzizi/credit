layui.config({
    ...
    tableIns =table.render({
        ...
        ,page: true
        ,cols:  [[ //标题栏
            {field: 'loginName', title: '登录号'}
            ,{field: 'loginSalt', title: '盐值'}
            ,{field: 'loginStatus', title: '登录状态',width:100}
            ,{field: 'loginType', title: '登录方式',width:100,toolbar: '#loginTypeBar'}
            ,{field: 'loginidBeformodify', title: '修改前登录用户号'}
            ,{field: 'lastLoginTime', title: '最后登录时间',width:150}
            ,{field: 'created', title: '创建时间',width:150}
            ,{field: 'updated', title: '更新时间',width:150}
            ,{field: 'operatorUpdate', title: '操作',fixed: 'right',align:'center', width:120,toolbar: '#barDemo'}
        ]]
        ,url: '/manage/tuserLoginInfo/selectList'
        ,response: {
    ...
    $(".search_btn").click(function () {
        tableIns.reload({
            where: { //设定异步数据接口的额外参数，任意设
                ...
                loginSalt: $(".search_input1").val(),
                loginStatus: $("#state_search").val(),
                loginType: $("#state_search1").val(),
            },page: {
                curr: 1 //重新从第 1 页开始
            }
        });
    ...
    table.on('tool(adminTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('确定删除【'+data.subTitle+'】?',{icon: 3,title:'系统提示'}, function(index){
                $.ajax({
                    url:"/manage/tuserLoginInfo/deleteById",
                    data: {
                        id:data.id
                    },
                    type:"POST",
