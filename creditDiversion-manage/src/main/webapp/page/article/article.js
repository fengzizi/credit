layui.config({
    ...
    tableIns =table.render({
        ...
        ,url: '/manage/tarticle/selectList'
        ,response: {
    ...
    table.on('tool(adminTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('确定删除【'+data.title+'】?',{icon: 3,title:'系统提示'}, function(index){
                $.ajax({
                    url:"/manage/tarticle/deleteById",
                    data: {
                        id:data.id
                    },
                    type:"POST",
...
function openRisk(id, title) {
    parent.layui.layer.open({
        ...
        maxmin: true,
        type: 2,
        content: "page/article/articleDetails.html?id=" + id +"&type=" + ((title == "查看") ? "select" : "update"),
