...
layui.config({
    ...
    if(id!=0){
        //加载数据
        $.get("/manage/tarticle/selectOne", {id: id}, function (result) {
            if (result.code == 1) {
                ...
                if(result.data!=null){
                    ...
                    if(result.data.image!=null&&result.data.image!=''){
                        var htm= '<span style="position: relative;display: inline-block;">' +
                            '<img src="' + result.data.image + '" style="height: 106px;" class="layui-upload-img">' +
    ...
    function submit(data,index){
        ...
        var url="/manage/tarticle/update";
        if(data.field.id==0){
            url="/manage/tarticle/add";
        }
        $.ajax({
            url:url,
            data: data.field,
