layui.config({
    ...
    tableIns =table.render({
        ...
        ,cols:  [[ //标题栏
            ...
            ,{field: 'userNick', title: '名字'}
            ,{field: 'userReal', title: '真实姓名'}
        ...
        ,url: '/manage/tuserPersonInfo/selectList'
        ,response: {
    ...
    $(".search_btn").click(function () {
        tableIns.reload({
            where: { //设定异步数据接口的额外参数，任意设
                userNick: $(".search_input").val(),
                userReal: $(".search_input2").val(),
                gender: $("#state_search").val(),
    ...
    table.on('tool(adminTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('确定删除【'+data.userPhone+'】?',{icon: 3,title:'系统提示'}, function(index){
                $.ajax({
                    url:"/manage/tuserPersonInfo/deleteById",
                    data: {
                        id:data.uid
                    },
                    type:"POST",
...
function openRisk(id, title) {
    parent.layui.layer.open({
        title: title + "用户个人信息详情",
        area: ['1000px','550px'],
        maxmin: true,
        type: 2,
        content: "page/userPersonInfo/userPersonInfoDetails.html?id=" + id +"&type=" + ((title == "查看") ? "select" : "update"),
        success: function (layero, index) {
            setTimeout(function () {
                layui.layer.tips('点击此处返回用户个人信息列表', '.layui-layer-setwin .layui-layer-close', {
                    tips: 3
                });
            }, 500)
        }
