layui.config({
    ...
    $(".search_btn").click(function () {
        tableIns.reload({
            where: { //设定异步数据接口的额外参数，任意设
                auditSwitch:$("#state_search").val(),
                osType:$("#states_search").val()
            },page: {
                curr: 1 //重新从第 1 页开始
            }
        });
    ...
    form.on('switch(sexDemo)', function(obj){
        ...
        $.ajax({
            url:"/manage/tversionControl/update",
            data: {
                id:this.value,
    ...
    table.on('tool(adminTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('确定删除该条记录?',{icon: 3,title:'系统提示'}, function(index){
                $.ajax({
                    url:"/manage/tversionControl/deleteById",
                    data: {
                        id:data.id
                    },
                    type:"POST",
...
function openRisk(id, title) {
    parent.layui.layer.open({
        ...
        maxmin: true,
        type: 2,
        content: "page/versionControl/versionControlDetails.html?id=" + id +"&type=" + ((title == "查看") ? "select" : "update"),
