layui.config({
    ...
    tableIns =table.render({
        ...
        ,cols:  [[ //标题栏
            ...
            ,{field: 'roleType', title: '类型',toolbar:"#roleTypeBar",width:110}
            ,{field: 'roleStatus', title: '状态',toolbar:"#roleStatusBar",width:110}
            ,{field: 'roleRemark', title: '备注'}
            ,{field: 'operatorUpdate', title: '操作',fixed: 'right',align:'center', width:170,toolbar: '#barDemo'}
        ]]
        ,url: '/manage/tmanageRoleInfo/selectList'
        ,response: {
    ...
    table.on('tool(adminTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){ //删除
            layer.confirm('确定删除【'+data.roleName+'】?',{icon: 3,title:'系统提示'}, function(index){
                $.ajax({
                    url:"/manage/tmanageRoleInfo/deleteById",
                    data: {
                        id:data.id
                    },
                    type:"POST",
...
function openRisk(id, title) {
    parent.layui.layer.open({
        ...
        maxmin: true,
        type: 2,
        content: "page/manageRoleInfo/manageRoleInfoDetails.html?id=" + id +"&type=" + ((title == "查看") ? "select" : "update"),
